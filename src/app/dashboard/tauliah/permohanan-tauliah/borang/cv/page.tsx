...
export default function BorangPPM() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const data = await getPersonalInfoFromCredentialApplication(user.id);          
        
      const mappedPersonalInfo = mapButirPeribadi(data);
      const mappedFamilyInfo = mapButirKelaurga(data);   
      const academic = await getAcademicDataAPI(user.id);
      const mappedOccupationInfo = mapButirPekerjaan(data);
      const serviceData = await getServiceDataAPI(user.id);
      const awardsPPM = await getAwardDataAPI(user.id, "PPM");
      const awardsNonPPM = await getAwardDataAPI(user.id, "nonPPM");
