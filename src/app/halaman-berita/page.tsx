...
import { Search, Calendar, Clock, TrendingUp, Tag } from "lucide-react"
...
export default function HalamanBerita() {
  ...
  const trendingNews = newsList.slice(0, 4)
      ...
      <div className="bg-white border-b">
        <div className="container mx-auto px-4 py-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            {/* PAGE TITLE */}
            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-2">Berita Pengakap Johor</h1>
              <p className="text-gray-600">Ikuti perkembangan terkini aktiviti Pengakap Johor</p>
            </div>
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-wrap gap-2">
            {categories.map((category) => (
              <Link
                key={category}
                href={category === "Utama" ? "/berita" : `/halaman-berita/${encodeURIComponent(category)}`}
              >
                <Button variant="outline" size="sm" className="rounded-full bg-white text-gray-900">
                  {category}
                </Button>
              </Link>
            ))}
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Berita Terkini</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-2">
                    {categories.slice(1).map((category) => (
                      <Link
                        key={category}
                        href={`/halaman-berita/${encodeURIComponent(category)}`}
                        className="flex items-center justify-between hover:bg-gray-50 p-2 rounded-lg transition-colors"
                      >
                        <span className="text-gray-600">{category}</span>
                        <Badge variant="secondary">
                          {newsList.filter((news) => news.category === category).length}
                        </Badge>
                      </Link>
                    ))}
                  </div>
...
function FeaturedNewsCard({ news }: { news: News }) {
  return (
    <Link href={`halaman-berita/${encodeURIComponent(news.title)}`}>
